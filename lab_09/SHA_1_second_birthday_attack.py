...
def main():
    flag = 0
    m1 = "realise the second type of birthday attack！"
    # 定义字符集合，这里假设是小写字母
    characters = string.printable
    # 生成所有长度为 2 的字符串
    all_strings = [''.join(comb) for comb in itertools.product(characters, repeat=2)]
    len_m1 = len(m1)
    collisions_dict = {}
    while True:
        # 对给定消息产生变形
        for insert_position_1 in range(len_m1):
            for s in all_strings:
                m2 = m1[:insert_position_1] + s + m1[insert_position_1:]
                hash2 = sha_1(m2)
                hash_prefix2 = hash2[:8]
                # print(m1)
                # print(m2)
                # print(hash1)
                # print(hash2)
                if hash_prefix2 in collisions_dict:
                    if m2 == collisions_dict[hash_prefix2]:
                        continue
                    else:
                        print(collisions_dict[hash_prefix2])
                        print(m2)
                        flag += 1
                        if flag >= 3:
                            break
                else:
                    collisions_dict[hash_prefix2] = m2
            if flag >= 3:
                break
        break
