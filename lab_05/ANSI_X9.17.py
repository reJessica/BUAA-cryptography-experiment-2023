...
def EDE(IV, k1, k2, X):
    R = des_encrypt(X, k1, 1)
    R = des_decrypt(R, k2, 1)
    R1 = des_encrypt(R, k1, 1)
    R = R1 ^ IV
    R = des_encrypt(R, k1, 1)
    R = des_decrypt(R, k2, 1)
    R = des_encrypt(R, k1, 1)
    V = R1 ^ R
    V = des_encrypt(V, k1, 1)
    V = des_decrypt(V, k2, 1)
    V = des_encrypt(V, k1, 1)
    return R, V
...
def main():
    ...
    n = int(input().strip())
    for i in range(n):
        T = int(input().strip(), 16)
        R, IV = EDE(IV, key_list1, key_list2, T)
